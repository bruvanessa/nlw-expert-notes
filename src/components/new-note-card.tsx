    ...
    <Dialog.Portal>
        <Dialog.Overlay className="inset-0 fixed bg-black/60"></Dialog.Overlay>
          <Dialog.Content className="fixed overflow-hidden left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 max-w-[640px] w-full h-[60vh] bg-slate-700 rounded-md flex flex-col">
            <Dialog.Close className="absolute right-0 top-0 outline-none bg-slate-800 p-1.5 text-slate-400 hover:text-slate-100">
              <X className="w-5 h-5"/>
            </Dialog.Close>
             ...
             <form className="flex-1 flex flex-col">
                ...
                Adicionar nota
                    ...
                    {shouldShowOnboarding ? (
                        ...
                        <textarea autoFocus className="text-sm leading-6 text-slate-480 bg-transparent resize-none flex-1 outline-none"
                        onChange={handleContentChanged} value={content}/>
                    )
                    }
                </div>

                {isRecording ? (<button type="button" onClick={handleStopRecording} className="w-full flex items-center justify-center gap-2 bg-slate-900 py-4 text-center texte-sm text-slate-300 font-medium hover:text-slate-100">
                   <div className="size-3 rounded-full bg-red-500 animate-pulse"></div>
                    Gravando! (clique p/ interromper)
                </button>) :(
                    <button type="button" onClick={handleSaveNote} className="w-full bg-lime-400 py-4 text-center texte-sm text-lime-950 font-medium hover:bg-lime-500">
                    Salvar nota
                </button>
                )} 
